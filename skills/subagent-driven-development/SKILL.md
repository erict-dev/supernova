...
Runtime note: if your agent runtime does not support delegation, keep the same implementer → spec reviewer → code reviewer sequence as separate sequential passes. See `docs/agent-runtime-compat.md`.
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "Mark task complete in tracker" [shape=box];
    }

    "Read plan, extract all tasks with full text, note context, create tracker items" [shape=box];
    ...
    "Read plan, extract all tasks with full text, note context, create tracker items" -> "Dispatch implementer subagent (./implementer-prompt.md)";
    ...
    "Code quality reviewer subagent approves?" -> "Mark task complete in tracker" [label="yes"];
    "Mark task complete in tracker" -> "More tasks remain?";
...
[Read plan file once: plans/supernova/<feature>/plan.md]
...
You: "User level (~/.config/supernova/hooks/)"
